...
metadata_filename="$1"
template='{"actor":"%s","event":"%s","head_ref":"%s","ref":"%s","run_id":"%s","sha":"%s"}'
json_string=$(printf "$template" "$GITHUB_ACTOR" "$GITHUB_EVENT_NAME" "$GITHUB_HEAD_REF" "$GITHUB_REF" "$GITHUB_RUN_ID" "$GITHUB_SHA")
