...
foreach(STLP_AP ${STLP_PIPELINES})
    ...
    if(${CMAKE_HOST_SYSTEM_NAME} STREQUAL Windows)
        add_custom_command(
            OUTPUT example_stlp_fat_int_${STLP_AP}.fs
            COMMAND ${CMAKE_COMMAND} -E make_directory %temp%/fatmktmp/fs
            COMMAND ${CMAKE_COMMAND} -E copy demo.txt %temp%/fatmktmp/fs/demo.txt
            COMMAND fatfs_mkimage --input=%temp%/fatmktmp --output=example_stlp_fat_int_${STLP_AP}.fs
        ...
        add_custom_command(
            OUTPUT example_stlp_fat_int_${STLP_AP}.fs
            COMMAND bash -c "tmp_dir=$(mktemp -d) && fat_mnt_dir=$tmp_dir && mkdir -p $fat_mnt_dir && mkdir $fat_mnt_dir/fs && cp ./demo.txt $fat_mnt_dir/fs/demo.txt && fatfs_mkimage --input=$tmp_dir --output=example_stlp_fat_int_${STLP_AP}.fs"
            DEPENDS example_stlp_int_${STLP_AP}
            COMMENT
                "Create filesystem"
            WORKING_DIRECTORY
    ...
    add_custom_target(flash_fs_example_stlp_int_${STLP_AP}
        COMMAND xflash --quad-spi-clock 50MHz --factory example_stlp_int_${STLP_AP}.xe --boot-partition-size 0x100000 --data ${CMAKE_CURRENT_LIST_DIR}/filesystem_support/example_stlp_fat_int_${STLP_AP}.fs
        DEPENDS example_stlp_fat_int_${STLP_AP}.fs
        COMMENT
            "Flash filesystem"
        VERBATIM
    )
