...
There are two ASRC blocks, one in the I2S \rightarrow ASRC \rightarrow USB path and the other in the USB \rightarrow ASRC \rightarrow I2S path.
...
The ASRC algorithm in the `lib_src <https://github.com/xmos/lib_src/>`_  library is used for the ASRC processing. The ASRC processing is block based and works on a block size of 244 samples per channel in the I2S \rightarrow ASRC \rightarrow USB path and 96 samples per channel in the USB \rightarrow ASRC \rightarrow I2S path.
